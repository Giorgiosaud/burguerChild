 ...
 * Draws works
 */
class zpMenuShortcode extends ctShortcodeQueryable implements ctVisualComposerShortcodeInterface
{
    ...
    public function handle($atts, $content = null)
    {
        ...
        $menuBoxHtml = '[menu_box_zp ' . $footer . ' ' . $footer_link . ' ' . $footer_link_text . '  style="' . $style . '" title="' . $categoryName . '" image="' . $category_image . '" description="' . $category_description . '"]';

        foreach ($products as $p) {
            $custom = get_post_custom($p->ID);
            $counter++;
            var_dump($custom["currency"]);
    ...
    public function getAttributes()
    {
        $atts = $this->getAttributesWithQuery(array(
            ...
            'style' => array('label' => __('Select style', 'ct_theme'), 'default' => '1', 'type' => 'select', 'options' => array(
                '1' => __('Rolled', 'ct_theme'),
                '2' => __('Scratch 1', 'ct_theme'),
                '3' => __('Scratch 2', 'ct_theme'),
                '4' => __('Sharp', 'ct_theme'),
            )),
	...
	public function getVisualComposerInfo() {
		return new ctVisualComposerInfo( $this, array(
		    'icon' => 'fa-book',
            'description'=>__("Display menu card", 'ct_theme'),
        ) );
	}
}
